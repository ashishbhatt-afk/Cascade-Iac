...
# terraform init -reconfigure -backend-config="resource_group_name=rg-statefile-poc-cascade" -backend-config="key=iis.dev.terraform.tfstate" -backend-config="storage_account_name=Enter your storage account name here" -backend-config="container_name=tfbackend" -var-file="dev.tfvars"
...
data "azurerm_key_vault_secret" "iisvmpassword" {
  name         = "iisvmpassword"
...
resource "azurerm_public_ip" "iis_public_ip" {
  count               = contains(["dev", "staging"], var.environment) ? var.web_count : 0
...
resource "azurerm_network_interface" "iis_nic" {
  ...
  ip_configuration {
    ...
    public_ip_address_id          = contains(["dev", "staging"], var.environment) ? azurerm_public_ip.iis_public_ip[count.index].id : null
  }
   tags = local.tags
}
...
module "vm" {
  ...
  vm_count                     = var.web_count
  vm_name_prefix               = var.vm_name_prefix
  location                     = data.terraform_remote_state.core.outputs.rglocation
  resource_group_name          = data.terraform_remote_state.core.outputs.resource-group-name
  vm_size                      = var.vm_size
  admin_username               = var.admin_username
  admin_password               = data.azurerm_key_vault_secret.iisvmpassword.value
...
resource "azurerm_network_interface_backend_address_pool_association" "nic_backend_association" {
  count                   = var.web_count
  network_interface_id    = azurerm_network_interface.iis_nic[count.index].id
...
resource "azurerm_virtual_machine_extension" "Postdeploymentscript" {
  ...
  settings = jsonencode({
    fileUris = [
      "Enter your blob storage URL path here"
    ]
